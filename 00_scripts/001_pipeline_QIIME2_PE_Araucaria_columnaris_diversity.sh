...
# FastQC/MultiQC raw -> Trimmomatic -> FastQC/MultiQC cleaned -> import QIIME 2
# -> DADA2 -> visualisations/exports -> arbre phylogenetique ASV.
#
# Marqueurs traites strictement separement :
#   /nvme/bio/data_fungi/Araucaria_columnaris_diversity/01_raw_data/16S
#   /nvme/bio/data_fungi/Araucaria_columnaris_diversity/01_raw_data/ITS
#
# Les noms de FASTQ ne doivent PAS contenir le marqueur : le marqueur est
# determine exclusivement par le sous-repertoire parent (16S ou ITS).
#
...
# Pour ITS, choisir les trunc-len seulement apres examen detaille de demux.qzv
# et verification du chevauchement des reads apres trimming.
...
# =============================================================================
# Les FASTQ sont lus directement depuis RAW_ROOT_DIR/16S et RAW_ROOT_DIR/ITS.
# Les sample IDs biologiques sont identiques entre marqueurs, mais les manifests,
# reads nettoyes et objets QIIME2 restent dans des dossiers distincts.
...
for sample_id, files in sorted(records.items()):
    ...
    # Format normal : Ac-ZONE-TRAITEMENT-SOUSBLOC-TEMPS-ANNEE
    # Exemple : Ac-A-D1-1A-T11-1
    # Blanc negatif : Ac-NEG-T11-1, sans sous-bloc ni traitement D/T.
...
# Table complete servant d'audit des associations sample-ID/FASTQ.
...
# Le manifest pointe vers les paired reads issus de Trimmomatic, dans le dossier
# specifique au marqueur. Si Trimmomatic est deja fait ailleurs, adapter ici.
...
# =============================================================================
# 2. VERIFICATION DES FASTQ BRUTS
# =============================================================================
for marker in "${MARKERS[@]}"; do
    set_marker_variables "${marker}"
...
for marker in "${MARKERS[@]}"; do
    ...
    [[ -f "${QIIME_CORE}/rep-seqs.qza" ]] || die "rep-seqs.qza absent pour ${marker}: activez RUN_DADA2=true ou fournissez ce fichier."

    # Aucun filtrage de contingence, aucun decontam et aucun filtrage de
    # prevalence : l'arbre est construit sur l'ensemble des rep-seqs DADA2.
