#!/usr/bin/env bash
# =============================================================================
# Taxonomie QIIME 2 amplicon 2025.7 : 16S V4 (SILVA) et ITS (UNITE)
...
# 16S déjà terminé le 2026-09-03 : les artefacts SILVA et le classifieur sont
# présents. Ces deux indicateurs empêchent tout re-téléchargement ou tout
# ré-entraînement 16S ; le code 16S est néanmoins conservé intégralement.
REBUILD_16S_CLASSIFIER=false
DOWNLOAD_16S_SILVA=false

# ITS : lancer le téléchargement UNITE et reconstruire le classifieur.
...
# UNITE v10.0, publication du 19-02-2025. Avec QIIME 2 amplicon 2025.7,
# q2-rescript get-unite-data attend la date de publication, et non "10.0".
...
command -v qiime >/dev/null 2>&1 || {
    echo "ERREUR : qiime introuvable dans ${QIIME2_ENV}" >&2
    exit 1
}
...
# q2-rescript est livré dans la distribution amplicon ; la vérification rend
# l'erreur explicite s'il est absent de l'environnement.
...
# 16S V4 : SILVA 138.2
#
# La totalité de ce bloc est conservée pour documenter la construction du
# classifieur déjà produite. Grâce aux deux paramètres false ci-dessus, aucun
# téléchargement SILVA, aucune extraction, aucun entraînement, aucune nouvelle
# assignation et aucun export 16S ne seront exécutés lors de cette relance.
# =============================================================================
if [[ "${DOWNLOAD_16S_SILVA}" == true || "${REBUILD_16S_CLASSIFIER}" == true ]]; then
    ...
    if [[ "${DOWNLOAD_16S_SILVA}" == true ]]; then
        ...
        # Le classifieur exige des séquences ADN. Les séquences SILVA
        # téléchargées par RESCRIPt peuvent être en ARN : conversion U -> T.
    ...
    if [[ "${REBUILD_16S_CLASSIFIER}" == true ]]; then
        ...
        qiime tools export \
            --input-path "${SILVA_V4_UNIQ_SEQS}" \
            --output-path "${EXPORT_DIR}/database/silva_515F_806R_uniq_seqs"
        qiime tools export \
            --input-path "${SILVA_V4_UNIQ_TAX}" \
            --output-path "${EXPORT_DIR}/database/silva_515F_806R_uniq_tax"
        qiime tools export \
            --input-path "${SILVA_CLASSIFIER}" \
            --output-path "${EXPORT_DIR}/classifier/silva_515F_806R_naive_bayes"
    fi
else
    log "16S : étape désactivée ; les références et le classifieur SILVA existants sont conservés sans modification."
fi

# =============================================================================
# ITS : UNITE v10.0, publication du 19-02-2025, Fungi, jeu dynamique
#
# Correction principale : le DOI UNITE ne doit pas être téléchargé comme un ZIP.
# q2-rescript get-unite-data récupère les données directement depuis l'API
# PlutoF/UNITE et crée les deux artefacts QIIME 2 nécessaires.
# =============================================================================
...
if [[ "${DOWNLOAD_ITS_UNITE}" == true ]]; then
    log "ITS : téléchargement/import UNITE ${UNITE_RELEASE_DATE} via q2-rescript (${UNITE_TAXON_GROUP}, ${UNITE_CLUSTER_ID}, singletons=${UNITE_SINGLETONS})"

    # Nettoyage des artefacts QIIME ITS qui seront régénérés.
    rm -f "${UNITE_SEQS}" "${UNITE_TAX}"

    # Nettoyage facultatif des résidus du téléchargement DOI erroné de la
    # version précédente du script ; aucune dépendance à curl ou unzip.
    rm -f "${DB_DIR}/UNITE_${UNITE_VERSION}_fungi_dynamic_refs.qza.zip"
    rm -rf "${DB_DIR}/unite_download"

    unite_singleton_opt=()
    if [[ "${UNITE_SINGLETONS}" == true ]]; then
        unite_singleton_opt=(--p-singletons)
    else
        unite_singleton_opt=(--p-no-singletons)
    fi

    qiime rescript get-unite-data \
        --p-version "${UNITE_RELEASE_DATE}" \
        --p-taxon-group "${UNITE_TAXON_GROUP}" \
        --p-cluster-id "${UNITE_CLUSTER_ID}" \
        "${unite_singleton_opt[@]}" \
        --o-sequences "${UNITE_SEQS}" \
        --o-taxonomy "${UNITE_TAX}"
fi
...
qiime tools export \
    --input-path "${UNITE_ITS_UNIQ_SEQS}" \
    --output-path "${EXPORT_DIR}/database/unite_ITS7_ITS4_uniq_seqs"
qiime tools export \
    --input-path "${UNITE_ITS_UNIQ_TAX}" \
    --output-path "${EXPORT_DIR}/database/unite_ITS7_ITS4_uniq_tax"
qiime tools export \
    --input-path "${UNITE_CLASSIFIER}" \
    --output-path "${EXPORT_DIR}/classifier/unite_ITS7_ITS4_naive_bayes"
